...
import GenericItem from "@/components/SearchResultItem.vue";
...
<template>
	<AppPage :show-back-button="false" title="Home">
		...
		<div id="home-page-content" class="ion-padding">
			<section v-for="{ title, style, items } in homeFeed" :key="title" class="home-feed-section">
				<h1>{{ title }}</h1>

				<div v-if="style.type === 'list'" class="home-feed-list">
					<div class="list-items" :style="{ '--list-rows': style.rows ?? 2 }">
						<GenericItem v-for="item in items" :key="item.id" :item class="list-item" />
					</div>
				</div>
