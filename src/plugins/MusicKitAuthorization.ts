...
const MusicKitAuthorization = registerPlugin<MusicKitAuthorizationPlugin>(
  "MusicKitAuthorization",
  {
    web: () =>
      import("./MusicKitAuthorization/web").then(({ MusicKitAuthorization }) =>
        new MusicKitAuthorization()
      ),
  },
);
...
export default {
  async authorize(): Promise<MusicKit.MusicKitInstance> {
    ...
    const { developerToken, musicUserToken } = await MusicKitAuthorization
      .authorize();
